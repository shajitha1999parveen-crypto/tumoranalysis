...
# Set this from evaluate_model.py's "Suggested threshold" output.
# Raising it makes the model require more confidence before saying "yes" (tumor),
# which reduces false "yes" results on normal scans, at some cost to catching real tumors.
THRESHOLD = 0.37


@st.cache_resource
def load_artifacts():
    model = tf.keras.models.load_model("models/tumor_model.keras")
...
if uploaded_file is not None:
    ...
    resized = image.resize(IMG_SIZE, Image.BILINEAR)  # match tf.image's default resize method
    arr = np.expand_dims(np.array(resized), axis=0)

    pred = model.predict(arr)[0][0]
    predicted_class = class_names[1] if pred > THRESHOLD else class_names[0]
